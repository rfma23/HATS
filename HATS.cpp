...
vector<vector<vector<vector<float>>>> normalise(vector<vector<vector<vector<float>>>> histograms, int event_counter){
	/*
    ...
    */
    ...
    for(int i = 0; i < (int)result.size(); i++){
		for(int j = 0; j < (int)result[i].size(); j++){
			for(int k = 0; k < (int)result[i][j].size(); k++){
				for(int h = 0; h < (int)result[i][j][k].size(); h++){
					result[i][j][k][h] = histograms[i][j][k][h]/(event_counter+0.1);
				}
			}
		}
	}
